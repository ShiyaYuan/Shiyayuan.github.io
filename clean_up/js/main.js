...
function init() {
  ...
  $('#about-btn').click(function () {
    ...
    $('header,.audio-control').addClass('cursor-default');
  });
  ...
  //custom cursor
  $('.name,#email,.audio-icon,#formless-twin-link').mouseenter(function () {
    $('#cursor').addClass('hide');
  })
  $('.name,#email,.audio-icon,#formless-twin-link').mouseleave(function () {
  ...
  //side bar
  gsap.set(".thumbnail-wrapper", {xPercent: 102});
...
function dom() {
  ...
  customCursor = throttle(customCursor, 0);
  ...
  ev_mousemove = throttle(ev_mousemove, 0);
  // throttle for ev_mousemove, max 50ms execute once
  fireLastMouseEvent = throttle(fireLastMouseEvent, 0);
...
window.addEventListener("load", function () {
  // $('#path-wrapper').addClass("unscrollable");
  init();

  setTimeout(function () {
    gsap.to('.customScrollbar', {opacity: 1});
  }, 10);
  ...
  setTimeout(function () {
    ...
    setTimeout(function () {
      $('#cursor').fadeIn(500);
      $(('#cursor')).html("scroll");
      // $('#path-wrapper').removeClass("unscrollable");
      setTimeout(function () {
        $('#cursor').fadeOut(1000);
      }, 3000)
    })
